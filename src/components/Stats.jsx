const STATS = [
  ...
  { value: '350+', label: 'ZENVÍXO Experts' },
  { value: '600+', label: 'Satisfied Clients' },
]

export default function Stats() {
  return (
    <section className="bg-ink py-16 border-t border-white/5">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div className="mt-14 grid grid-cols-1 gap-10 sm:grid-cols-3">
          {STATS.map((s) => (
            <div key={s.label}>
              <p className="font-display text-5xl font-extrabold bg-gradient-to-r from-brand to-accent bg-clip-text text-transparent inline-block">{s.value}</p>
              <p className="mt-2 text-lg text-white/50">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
